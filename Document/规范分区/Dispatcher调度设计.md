...
`TaskType` 只允许 `reason|explore|judge|report|fork_seed`。Worker 配置中出现 `bootstrap` 属于非法配置。
...
Fork seed 是 ephemeral job，不 claim project reason lease。它读取 `input_snapshot_yaml`，调用 `default/recon/fork_seed.md`，由 AI 生成 child vuln 的初始 `seed_facts`。
...
8. 取消 inactive/deleted projects 上的本地运行任务，但 stopped project 上正在运行的 judge/fork_seed 不取消。
9. 为 stopped/completed projects 排队 container cleanup；如果 stopped project 正在运行 judge/fork_seed，则暂缓 stopped-container cleanup。
10. 调度可执行的 project tasks。
11. 调度 queued judge jobs 和 fork_seed jobs。
...
- Stopped recon projects 仍允许执行 queued judge 和 fork_seed ephemeral jobs。
- Stopped projects 执行 stopped-container cleanup；如果该 project 正在运行 judge/fork_seed，则等任务完成后再清理。
