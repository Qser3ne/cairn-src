...
1. 前端创建项目时提交 `project_kind` 和项目级 `auth_mode`；未传 `project_kind` 时服务端默认 recon。
...
3. Scheduler 对初始项目直接 dispatch reason，不创建 bootstrap intent。
...
- 完整测试：从 `cairn/` 目录运行 `pytest -q tests`；当前环境缺少 `uv` 时使用项目外临时 venv。
