...
- `CreateProjectRequest` 和 `ForkVulnRequest` 在 `auth_mode="authenticated"` 时要求至少一个 account。
- `auth_mode!="authenticated"` 时提交 accounts 返回 422。
...
- authenticated explore 派发前先领取空闲账号；领取失败则不 claim worker，不提交任务，只入队。
...
- reason 根据 `project.auth_mode` 规划方向：anonymous 只规划未登录攻击面，authenticated 只规划登录态攻击面。
