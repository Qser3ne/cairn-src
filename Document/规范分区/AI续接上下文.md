...
- `.agents/skills/production-deploy` 是项目级主动触发 skill，用于指导使用 `scripts/deploy.sh` 将 `/home/qser3ne/Application/carin-dev` 发布到 `/home/qser3ne/Application/carin`，并强调不覆盖生产 `dispatch.yaml`、`datas/` 等本地状态。
- `.agents/skills/git-release` 是项目级主动触发 skill，用于执行一次 release 提交；未指定版本时默认从最新 `vX.Y` tag 增加 `0.1`，提交说明概括相较于上个版本的更新内容。
