...
`project_kind="recon"` 是信息收集项目，目标是为后续漏洞验证准备高质量上下文。Recon 采用功能理解优先策略：先理解页面/功能、用户动作和业务流程，再把 route/API、参数和资产挂到功能点下。Recon 只收集：

- 资产和子资产。
- 页面功能、用户动作、业务流程、端点、入口和参数。
- 匿名和登录态认证边界。
- 候选攻击面和可疑但未验证的线索。
...
- 在 recon 中，reason 优先提出 `feature_mapping`、`workflow_mapping`、`feature_api_binding` 等功能地图 intents，再补资产发现、入口采样、认证边界识别、候选攻击面整理和噪声排除 intents。
- 在 vuln 中，reason 可以围绕 fork seed 功能点、已复制的 recon facts 和已有 findings 提出验证或跟进 intents；返回 noop 前必须检查所有有价值 feature seed 是否已覆盖或被明确排除。
- Reason 可以返回 noop/stable 决策，但不能返回 `complete`。

`explore` 负责执行一个已 claim 的 intent：

- Recon explore 写入已确认的事实，例如页面功能、可见用户动作、截图/DOM 证据、route/API 绑定、端点存在、登录后可访问、某入口为第三方服务、某参数值得后续验证。
- Recon explore 不验证漏洞，不创建 finding，不触发 report。
- Recon explore 的 `description` 保持兼容字段，但功能面 intent 可以额外写 `fact_type="feature_surface"`、`title`、`summary` 和 `details`；`details` 常见字段包括 `user_actions`、`routes`、`apis`、`evidence_refs`、`screenshot_refs` 和 `dom_refs`。
...
- `ready`：`score >= 75`，没有 blocking gaps，且至少存在可用于 vuln fork 的具体候选攻击面；核心功能不能只停留在端点清单，需说明页面/流程做什么、用户能做什么、route/API 如何服务该功能。
...
- AI fork planner 基于 recon snapshot 生成多个面向漏洞验证的 seed facts，并在每条 seed fact 中保留 `derived_from` 来源 fact IDs；功能点 seed 会保留 `feature_summary`、`user_actions`、`routes`、`apis`、验证重点、约束和证据引用。
