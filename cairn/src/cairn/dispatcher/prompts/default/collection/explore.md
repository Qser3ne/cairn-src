...
你将收到一份 collection task（信息收集任务）的 YAML 任务图快照和一个 Current Intent。只执行当前 collection intent，并返回一个新的客观 collection fact。不要切换任务，不要验证或报告漏洞。
...
`data` 允许包含 `description`、`fact_type`、`title`、`summary`、`details`。不要输出 `findings`，不要输出 `report`，不要输出 `complete`、`intents` 或其他字段。
...
- `data.description` 中的事实、证据说明、限制说明和下一步 collection 建议，建议优先使用简体中文；资产名、URL、端点、路径、参数名、命令、状态码和技术缩写可以保留英文。
...
- 记录有用的 collection 证据：页面截图、DOM 片段、可见功能、用户动作、route/API 绑定、资产列表、端点样本、认证边界、噪声排除项、范围说明和候选攻击面。
- 做功能面 collection 时，应先用真实浏览器截图和 DOM/可见文本理解“页面能做什么”，再把 route/API 挂到功能点下；不要只输出接口清单。
...
- <本次 intent 新确认的 collection 事实；只写增量事实>
evidence_refs:
...
suggested_next_collection:
- <仍属于 collection 的下一步建议；不得建议漏洞验证、finding 或 report>
```
