...
你将收到一份 YAML 格式的任务图快照，以及一个指定的当前 Intent。当前 validation task（漏洞验证任务）处于 SRC 漏洞挖掘模式。你只需要围绕当前 Intent 的方向进行探索，并产出新的增量事实；如果确认存在可提交的 SRC 漏洞，也可以在 JSON 中附带 findings 数组。
