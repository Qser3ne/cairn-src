...
Client / UI / API
        ...
        v
Cairn Server
  project graph, accounts, snapshots, findings, reports, ephemeral jobs
        ^
        | HTTP API
        v
Cairn Dispatcher
...
- Server 负责项目状态、graph 一致性、账号池数据、snapshot、finding、report 和 ephemeral job 的持久化。
...
1. 用户创建 recon 项目，提交 title、origin、hints 和至少一个账号。Server 固定写入 `auth_mode="dual"`，并只创建初始 `origin` fact。
...
## Reason / Explore / Judge responsibilities
...
- Judge 输出 verdict、score、recommended action、checklist、blocking gaps 和 non-blocking gaps。
...
## Anonymous vs authenticated recon lines

Recon 固定使用 `auth_mode="dual"`，通过 intent 级 `auth_scope` 拆分两条信息收集线路：

- `auth_scope="anonymous"`：不领取账号，不允许登录，不使用账号凭据，只观察匿名可访问资产和端点。
- `auth_scope="authenticated"`：从项目账号池领取一个账号，在该账号隔离的浏览器/session 状态中探索登录态攻击面。

Account pool 由 server 存储账号数据，由 dispatcher 做 intent 级租约调度：

- Reason、judge、report 和 anonymous explore 不领取账号。
- Authenticated explore 需要可用项目账号；没有空闲账号时进入 FIFO 等待队列，不提前 claim intent。
- 每个 authenticated explore 只绑定一个账号，任务结束、失败、取消或崩溃后释放租约。
- 账号隔离是 worker session 级边界，避免不同账号的认证状态互相污染。
...
- Authenticated explore 无可用账号时不 claim intent，只进入等待队列。
- Judge claim 后失败会写 ephemeral job error；如果因为项目 stopped/completed/deleted 等 inactive 状态被取消，也必须 fail 该 ephemeral job，避免 UI 长期显示 running。
- Report 失败时释放 report intent；成功时由 server 创建 finding report draft 并更新 finding `report_status`。

Routine heartbeat 和稳定轮询不应产生大量噪声日志；调度、claim conflict、timeout、parse failure、账号租约 wait/release、judge finish/fail 等状态变化应可观测。
...
- Judge 输出 ephemeral readiness judgement；不写 graph。
- Report 输出 `report_markdown` 和可选 `report_json`，由 dispatcher 通过 server report API 写入 finding report draft。

更细的 dispatcher 调度、prompt 加载和写回 API 约定见 [`dispatcher-design.md`](./dispatcher-design.md) 与 [`server-protocol.md`](./server-protocol.md)。
